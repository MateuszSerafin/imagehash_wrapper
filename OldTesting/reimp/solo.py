...
zus = 0
video_capture = cv2.VideoCapture("Rick and Morty S02E02 Mortynight Run.mp4")
for rand in random.sample(range(0,30000),100):

    video_capture.set(1, rand)
    ret, frame = video_capture.read()

    frame = PIL.Image.fromarray(frame)
    other_hash = crop_resistant_hash(frame, hash_func=imagehash.phash)
