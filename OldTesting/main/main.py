...
def crop_resistant_hash(
    ...
    for segment in segments:
        ...
        hashes.append(hash_func(bounding_box, hash_size=20))
    # Show bounding box
...
for file in os.listdir():
    if(not file.endswith(".bz2")): continue
    list.append(pickle.load(bz2.BZ2File(file, 'rb')))
    names.append(file)

bruh = PIL.Image.open("Bez tytułu.png")

other_hash = crop_resistant_hash(bruh)

for i in range(len(names)):
    a = list[i]
    print(names[i], " : ", a.index(other_hash.best_match(a)))
