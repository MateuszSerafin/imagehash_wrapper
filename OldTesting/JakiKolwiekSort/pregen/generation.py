...
def save(fileorg, what):
    file = None
    where = "8multihash/"
    if(not os.path.exists(os.path.join(where,os.path.basename(fileorg) + f""".bz28withresistant"""))):
        file = bz2.BZ2File(os.path.join(where, os.path.basename(fileorg) + f""".bz28withresistant"""), 'wb')
    else:
        file = bz2.BZ2File(os.path.join(where, os.path.basename(fileorg) + f""".bz28withresistant"""), 'ab')
...
if __name__=="__main__":
    correct_files = [".mp4", ".mkv"]
    multiprocessing.set_start_method('forkserver')
    pool = multiprocessing.Pool(16, maxtasksperchild=8)
    for root, dir, files in os.walk("videos"):
        for file in files:
            filename, file_extension = os.path.splitext(file)
            if(file_extension not in correct_files): continue
            if(os.path.exists(os.path.join("8multihash/", (filename + file_extension + ".bz28withresistant")))):
                print("PROCESSED ARLEADY :", filename)
                continue
            processfile(pool, os.path.join(root,file))
